        					...
        					<tr>
        						...
        						<td>${hd.key.address }</td>
        						 ...
        						 <td>
        						 	...
        						 	<%-- Admin --%>
        						 	<c:choose>
        						 		<c:when  test="${ hd.key.giaoHang==-3 && user.admin==1}">
        						 		 Xác nhận hoàn trả <hr>
        						 		 <div> <a href="/java5_Lab5/hoaDon/trangthaigiaohang/hoanTra/${hd.key.id}" class="btn btn-primary">Oke</a> 
        						 		 		<a  href="/java5_Lab5/hoaDon/trangthaigiaohang/danggiao/${hd.key.id}" class="btn btn-danger">Hủy</a>]
        						 		 </div>
        						 	</c:when>
        						 	</c:choose>
        						 	<c:choose>
	        						...
	        						<%-- Admin --%>
		        						<c:choose>
		        							<c:when test="${hd.key.giaoHang==0 && user.admin==1 }"> <a href="/java5_Lab5/hoaDon/trangthaigiaohang/xacnhan/${hd.key.id}" class="btn btn-primary">Xác nhận</a> </c:when>
		        							<c:when test="${hd.key.giaoHang==1 && user.admin==1}"><a href="/java5_Lab5/hoaDon/trangthaigiaohang/danggiao/${hd.key.id}" class="btn btn-primary">Đang giao</a></c:when>
		        							<c:when test="${hd.key.giaoHang==-2 && user.admin==1}"><a href="/java5_Lab5/hoaDon/trangthaigiaohang/huyDon/${hd.key.id}" class="btn btn-primary">Đã nhận</a></c:when>
		        						</c:choose>
	    	        					<c:if test="${ user.admin==1 }"><a href="/java5_Lab5/hoaDon/trangthaigiaohang/huydon/${hd.key.id }" class="btn btn-danger">Hủy đơn</a></c:if>
